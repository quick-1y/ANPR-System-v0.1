...
### OCR
- **CRNN** (INT8, квантизованный через `torch.ao.quantization.quantize_fx`) считывает символы с кропа номерной пластины.
...
- Все параметры (пути к моделям/БД, каналы, сетка, `tracking.best_shots`, `tracking.cooldown_seconds`) лежат в `settings.json` и управляются через `settings_manager.py`.
...
- `settings.json` — хранит конфигурацию каналов, сетки, параметр `tracking.best_shots` для агрегации по трекам и `tracking.cooldown_seconds` для подавления повторных срабатываний.
